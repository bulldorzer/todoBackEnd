...
public interface ProductRepository extends JpaRepository<Product, Long> {
  ...
  // SELECT P.* FROM Product p where p.pno = ? [JPQL]
  ...
  @Query("select p, pi from Product p left join p.imageList pi on pi.ord = 0 where p.keyword like concat('%', :keyword, '%') and p.delFlag = false")
